...
class ShootingNode():
    def __init__(self, pd: ProblemData, contactIds):
        ...
        self.integrate = casadi.Function('xplus', [cx, cdx],
                                         [casadi.vertcat(cpin.integrate(cmodel, cx[:pd.nq], cdx[:pd.nv]),
                                                         cx[-pd.nv:]+cdx[-pd.nv:])])
        ...
        self.difference = casadi.Function('xminus', [cx, cx2],
                                          [casadi.vertcat(cpin.difference(cmodel, cx2[:pd.nq], cx[:pd.nq]),
                                                          cx2[pd.nq:]-cx[pd.nq:])])
    ...
    def constraint_standing_feet_cost(self):
        ...
        for i, stFoot in enumerate(self.contactIds):
            ...
            A = np.matrix([[1, 0, -self.pd.mu], [-1, 0, -self.pd.mu], [6.1234234 *
                          1e-17, 1, -self.pd.mu], [-6.1234234*1e-17, -1, -self.pd.mu], [0, 0, 1]])
            lb = np.array([-casadi.inf, -casadi.inf, -
                          casadi.inf, -casadi.inf, 0])
            ub = np.array([0, 0, 0, 0, casadi.inf])
            r = A @ fw
            self.cost += self.pd.friction_cone_w * \
                casadi.sumsqr(casadi.if_else(r <= lb, r, 0))/2 * self.dt
            self.cost += self.pd.friction_cone_w * \
                casadi.sumsqr(casadi.if_else(r >= ub, r, 0))/2 * self.dt
    ...
    def body_reg_cost(self, x_ref):
        if self.isTerminal:
            ...
            self.cost += 1/2 * \
                casadi.sumsqr(self.pd.state_reg_w *
                              self.difference(self.x, x_ref)) * self.dt

    def target_cost(self, target):
        # I am Assuming just FR FOOt to be free
        for sw_foot in self.freeIds:
            self.cost += 1/2 * self.pd.foot_tracking_w * \
                casadi.sumsqr(self.feet[sw_foot](self.x) - target) * self.dt
